 ...
 *
 * The collectors (collectUplinkLOBs / collectDownlink) are shared with
 * tactical mode, mirroring the upstream helper extraction; doScans below is
 * the orbit-mode scan pass, doScansTactical lives in tactical.ts.
 *
...
import type { Drone, Team, TrackPoint } from "./types.js";
...
export function collectDownlink(sim: SimCtx, T: Team, d: Drone): TrackPoint | null {
  const CUAS = sim.config.CUAS;
  const side = T.side;
