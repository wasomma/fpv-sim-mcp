 ...
 *
 * collectUplinkLOBs() / collectDownlink() are one scan each and are shared
 * by both modes (upstream extracted them the same way); doScans() is the
 * orbit-mode scan, tactical.ts has its own.
 */
...
import type { Drone, DroneTrack, Team } from "./types.js";

// One DF scan for team T against emitter E's C2 uplink: each node that has
// the enemy GCS in range rolls to detect, and a detection appends a noisy
// LOB to T's collection effort. Shared by both modes.
export function collectUplinkLOBs(sim: SimCtx, T: Team, E: Team): void {
  const CUAS = sim.config.CUAS, side = T.side;
...
// One DF scan for team T against an enemy air vehicle d's video downlink.
// Returns the noisy track from the last node that detected it this scan
// (null if none). Shared by both modes.
export function collectDownlink(sim: SimCtx, T: Team, d: Drone): DroneTrack | null {
  const CUAS = sim.config.CUAS, side = T.side;
  let track: DroneTrack | null = null;
  for (const nd of T.nodes) {
    ...
    if (sim.rng() < p) {
      nd.dl++;
      track = { x: d.x + sim.gauss() * 55, y: d.y + sim.gauss() * 55, t: sim.t };
      if (!T.flags.dlFirst) {
...
export function doScans(sim: SimCtx): void {
  for (const side of ["BLUFOR", "OPFOR"] as const) {
    ...
    if (videoActive(E, sim.t)) {
      const track = collectDownlink(sim, T, E.drone!);
      if (track) T.droneTrack = track;
    }
  }
}
