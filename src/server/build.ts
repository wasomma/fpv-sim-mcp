/*
 * fpv-sim-mcp — server construction, shared by every transport entry point.
 *
 * buildServer() registers the five tools and two documentation resources on a
 * fresh McpServer. Every call constructs a fresh Simulation, so concurrent
 * calls cannot contaminate each other and identical inputs always return
 * identical outputs. The stdio entry (index.ts) builds one server for the
 * life of the process; the HTTP entry (http.ts) builds one per request,
 * which the engine's statelessness makes safe.
 *
...
import {
  ...
  type ConfigOverrides, type EngagementResult, type SimMode,
...
function sweep(startSeed: number, count: number, overrides?: ConfigOverrides, mode?: SimMode): EngagementResult[] {
...
export function buildServer(): McpServer {
  ...
  server.registerTool(
    "run_engagement",
    {
      ...
      description:
        ...
        "Tactical mode adds the objective, sortie tallies and the per-airframe package state. Notional data.",
      inputSchema: {
        seed: seedSchema,
        mode: modeInput,
  ...
  server.registerTool(
    ...
    async ({ start_seed, count, mode, config_overrides }) =>
      json(aggregateSweep(sweep(start_seed, count, config_overrides, mode))),
  );
  ...
  server.registerTool(
    ...
    async ({ start_seed, count, mode, config_a, config_b, label_a, label_b }) => {
      ...
      const runsA = sweep(start_seed, count, config_a, mode);
      const runsB = sweep(start_seed, count, config_b, mode);
