...
events, float-equal CEPs, no epsilons. The fixture holds both engagement
modes in one file (`runs[].mode`: five `orbit` runs first, then six
`tactical`), so both repos' CI gates, which compare `runs` verbatim,
cover both modes; `src/engine/tactical.ts` is the port of upstream's
`TACTICAL MODE` section and must mirror it call site by call site.
Policy:
...
- `npm test` — build + 24 tests (golden parity in both modes, fix math,
  aggregation, reported version).
